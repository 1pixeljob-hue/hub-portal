...
try {
    ...
    foreach (\ as \) {
        \ = \['color'];
        if (strpos(\, '#') === 0) {
            \ = "text-[{\}] bg-[{\}]/10 dark:bg-[{\}]/20";
        }
        else {
            \ = 'text-' . \ . '-500 bg-' . \ . '-50 dark:bg-' . \ . '-500/10';
        }
        \[\['id']] = [
            'name' => \['name'],
            'icon' => \['icon'],
...
<p class="text-slate-900 dark:text-white text-3xl font-bold"><?php echo \; ?></p>
</div>
<div class="glass-card rounded-xl p-5 flex flex-col gap-2 relative overflow-hidden group">
<div class="absolute top-0 right-0 w-24 h-24 bg-primary/10 rounded-full blur-2xl -mr-10 -mt-10 transition-transform group-hover:scale-150"></div>
<div class="flex items-center gap-2 mb-1">
...
<p class="text-slate-900 dark:text-white text-3xl font-bold"><?php echo count(\); ?></p>
</div>
<div class="glass-card rounded-xl p-5 flex flex-col gap-2 relative overflow-hidden group">
<div class="absolute top-0 right-0 w-24 h-24 bg-primary/10 rounded-full blur-2xl -mr-10 -mt-10 transition-transform group-hover:scale-150"></div>
<div class="flex items-center gap-2 mb-1">
...
<div class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-primary border border-slate-200 dark:border-slate-700">
    <?php if (\['logoUrl']): ?>
        <img src="<?php echo htmlspecialchars(\['logoUrl']); ?>" class="w-6 h-6 object-contain" />
    <?php else: ?>
        <span class="material-symbols-outlined"><?php echo htmlspecialchars(\[\['theme']]['icon'] ?? 'link'); ?></span>
    <?php endif; ?>
</div>
<div>
<h3 class="text-slate-900 dark:text-white font-semibold line-clamp-1"><?php echo htmlspecialchars(\['title']); ?></h3>
<p class="text-slate-500 dark:text-slate-400 text-xs mt-0.5"><?php echo htmlspecialchars(parse_url(\['url'], PHP_URL_HOST) ?? \['url']); ?></p>
</div>
</div>
...
<div class="flex flex-wrap gap-2 mb-5 mt-auto">
    <?php foreach(getTags(\['tags']) as \): ?>
        <span class="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-medium border border-slate-200 dark:border-slate-700"><?php echo htmlspecialchars(\['name'] ?? \); ?></span>
    <?php endforeach; ?>
</div>
<a href="<?php echo htmlspecialchars(\['url']); ?>" target="_blank" class="w-full flex items-center justify-center gap-2 bg-transparent hover:bg-primary/5 text-primary border border-primary/20 hover:border-primary/50 py-2.5 rounded-xl text-sm font-semibold transition-all">
